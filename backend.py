...
def events_overview_page():
    """SideQuest Events – Single-scroll, Webflow-y aesthetic with parallax + tasteful transitions"""
...
<head>
  ...
  <style>
    /* --------- Design tokens --------- */
    :root{
      --bg:#0a0a0a; --bg-2:#101010; --panel:#131316; --text:#fafafa; --muted:#9a9a9a;
      --primary:#FFD700; --accent:#FF6B35; --purple:#8B5FBF; --border:rgba(255,255,255,.08);
      --ring: rgba(255,215,0,.18);
      --radius:22px;
      --wrap: min(1400px, 92vw);
      --space: clamp(48px, 7vw, 110px);
    }
    *{box-sizing:border-box} html{scroll-behavior:smooth}
    ...
    /* --------- Gradient mesh backdrop (Awwwards-y) --------- */
    ...
    .blob{position:absolute;width:60vmax;height:60vmax;border-radius:50%;
      background:radial-gradient(closest-side, rgba(255,215,0,.18), transparent 70%)}
    .blob.b1{left:-10vmax;top:-6vmax}
    ...
    .blob{animation:floaty 12s ease-in-out infinite}

    /* --------- Opening reveal (failsafe below) --------- */
    .reveal{position:fixed;inset:0;background:var(--bg);z-index:9999;display:flex;align-items:center;justify-content:center;flex-direction:column;transition:opacity .8s ease,visibility .8s ease}
    ...
    /* Cards (no shimmer, just clean lift + soft ring) */
    ...
    .banner::after{content:'';position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,.45),transparent 55%)}

    /* Masked image reveal on scroll */
    .mask{--r:18px;mask-image:radial-gradient(120% 120% at 50% 0%, #000 40%, transparent 60%);mask-composite:exclude}
    ...
    .body{padding:22px}
    .pill{display:inline-block;padding:6px 12px;border-radius:999px;font-size:.72rem;font-weight:900;letter-spacing:.08em;margin-bottom:12px;background:var(--ring);color:#ffd86a}
    ...
    .cal-banner::after{content:'';position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,.45),transparent 55%)}
    ...
    /* Section separators (soft gradient stripes) */
    ...
    .divider::before{content:'';position:absolute;inset:0;background:
      ...
      radial-gradient(40% 120% at 50% 100%, rgba(255,107,53,.08), transparent 60%);
      opacity:.6;filter:blur(12px)}
...
<body>
  ...
  <script>
    ...
    function bannerFor(title){if(!title) return GAME_IMAGES['generic']||'/static/games/generic.jpg';const k=String(title).toLowerCase().trim();if(GAME_IMAGES[k])return GAME_IMAGES[k];for(const t of Object.keys(GAME_IMAGES)){if(k.includes(t))return GAME_IMAGES[t];}return GAME_IMAGES['generic']||'/static/games/generic.jpg';}
      ...
      try{
        ...
        if(j.success && j.events.length){
          grid.innerHTML=j.events.map(ev=>{
            ...
            const spots=cap?Math.max(cap-reg,0):null; let pill=''; let text='Open Registration';
    ...
    async function loadSpecialEvents(){
      ...
      try{
        ...
        if(j.success && j.events.length){
          grid.innerHTML=j.events.map(ev=>{
            const dt=new Date(ev.date_time),reg=ev.registration_count||0,cap=(ev.capacity||0)>0?ev.capacity:null;
            const spots=cap?Math.max(cap-reg,0):null; let pill=''; let text='Available';
            if(spots!==null){ if(spots===0){pill='warn';text='Full'} else if(spots<=3){pill='soon';text=spots+' Spots Left'} }
    ...
    /* ---------- Boot ---------- */
    (function boot(){
      // Load hero stats quickly, then section content lazily as it enters view
      loadStats();
      const todo = new Map([
      ...
      // Load tournaments immediately so first fold isn't empty
      loadTournaments();
