...
# Add this to your backend.py file - enhance the run_database_migrations function
...
def run_database_migrations():
    ...
    # Define all migrations (add the new analytics ones)
    migrations = [
        {
            'version': 1,
        ...
        # NEW ANALYTICS MIGRATION
        {
            'version': 4,
            'description': 'Add analytics tracking tables',
            'sql': [
                ...
                '''CREATE TABLE IF NOT EXISTS subscriber_source_analytics (
                    id SERIAL PRIMARY KEY,
                ...
                '''CREATE TABLE IF NOT EXISTS event_performance_cache (
                    id SERIAL PRIMARY KEY,
                    ...
                    avg_capacity_util DECIMAL(5,2) DEFAULT 0,
                    last_updated TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    UNIQUE(event_type)
                );'''
            ]
...
def calculate_growth_rate():
    """Calculate subscriber growth rate (current week vs previous week)"""
    try:
        # Get current week and previous week subscriber counts
        query = """
            SELECT 
                DATE_TRUNC('week', date_added) as week,
                COUNT(*) as signups
            FROM subscribers 
        ...
        current_week = result[0]['signups']
        previous_week = result[1]['signups']
        ...
        log_error(f"Error calculating growth rate: {e}")
        return 0

def calculate_event_conversion_rate():
    """Calculate percentage of subscribers who have attended events"""
    try:
        query = """
            SELECT 
                COUNT(DISTINCT s.email) as total_subscribers,
                COUNT(DISTINCT r.subscriber_email) as event_attendees
            FROM subscribers s
            LEFT JOIN event_registrations r ON s.email = r.subscriber_email AND r.attended = true
        """
        
        result = execute_query_one(query)
        
        if not result or result['total_subscribers'] == 0:
            return 0
        
        conversion_rate = (result['event_attendees'] / result['total_subscribers']) * 100
        return round(conversion_rate, 1)
        
    except Exception as e:
        log_error(f"Error calculating conversion rate: {e}")
        return 0

def calculate_engagement_rate():
    """Calculate percentage of subscribers who have registered for events"""
    try:
        query = """
            SELECT 
                COUNT(DISTINCT s.email) as total_subscribers,
        ...
        engagement_rate = (result['active_subscribers'] / result['total_subscribers']) * 100
        return round(engagement_rate, 1)
        
    except Exception as e:
        log_error(f"Error calculating engagement rate: {e}")
        return 0

def calculate_avg_revenue_per_event():
    """Calculate average revenue per event"""
    try:
        query = """
            SELECT 
                AVG(e.entry_fee * COALESCE(reg_count.count, 0)) as avg_revenue
            FROM events e
            LEFT JOIN (
        ...
        if not result or not result['avg_revenue']:
            return 0
        
        return round(float(result['avg_revenue']), 2)
        
    except Exception as e:
        log_error(f"Error calculating avg revenue: {e}")
        return 0

def calculate_attendance_rate():
    """Calculate percentage of registrations that resulted in attendance"""
    try:
        query = """
            SELECT 
        ...
        log_error(f"Error calculating attendance rate: {e}")
        return 0

def calculate_capacity_utilization():
    """Calculate average capacity utilization across events"""
    try:
        query = """
            SELECT 
                AVG(
                    CASE 
                        WHEN e.capacity > 0 THEN 
                            (COALESCE(reg_count.count, 0)::float / e.capacity * 100)
                        ELSE 0 
                    END
                ) as avg_capacity_util
            FROM events e
            LEFT JOIN (
                SELECT event_id, COUNT(*) as count
                FROM event_registrations
        ...
        if not result or not result['avg_capacity_util']:
            return 0
        
        return round(float(result['avg_capacity_util']), 1)
        
    except Exception as e:
        log_error(f"Error calculating capacity utilization: {e}")
        return 0
...
def get_analytics_kpis():
    ...
    try:
        kpis = {
            ...
            'event_conversion': calculate_event_conversion_rate(),
            'engagement_rate': calculate_engagement_rate(),
            'avg_revenue': calculate_avg_revenue_per_event(),
        ...
        log_error(f"Error getting KPIs: {e}")
        return jsonify({"success": False, "error": str(e)}), 500

@app.route('/api/analytics/subscriber-growth', methods=['GET'])
def get_subscriber_growth():
    ...
    try:
        ...
        query = """
            SELECT 
                ...
                (
                    SELECT COUNT(*) 
                    FROM subscribers s2 
                    WHERE DATE(s2.date_added) <= DATE(s1.date_added)
                ) as cumulative_total
            FROM subscribers s1
        ...
        growth_data = execute_query(query, (days,))
        
        # Fill in missing dates with zero signups
        filled_data = []
        current_date = datetime.now().date() - timedelta(days=days-1)
        current_cumulative = 0
        
        # Get baseline cumulative count
        baseline_query = """
            SELECT COUNT(*) as baseline
            FROM subscribers 
            WHERE date_added < %s
        """
        baseline_result = execute_query_one(baseline_query, (current_date,))
        current_cumulative = baseline_result['baseline'] if baseline_result else 0
        
        for i in range(days):
            date_str = current_date.isoformat()
            day_data = next((d for d in growth_data if str(d['date']) == date_str), None)
            
            if day_data:
                daily_signups = day_data['daily_signups']
                current_cumulative += daily_signups
            else:
                daily_signups = 0
        ...
        return jsonify({
            "success": True,
            "growth_data": filled_data
        })
        
    except Exception as e:
        log_error(f"Error getting subscriber growth: {e}")
        return jsonify({"success": False, "error": str(e)}), 500

@app.route('/api/analytics/event-performance', methods=['GET'])
def get_event_performance():
    """Get event performance data by type"""
    try:
        query = """
            SELECT 
                e.event_type,
                ...
                SUM(e.entry_fee * COALESCE(reg_counts.reg_count, 0)) as total_revenue
            FROM events e
        ...
        performance_data = execute_query(query)
        ...
        log_error(f"Error getting event performance: {e}")
        return jsonify({"success": False, "error": str(e)}), 500

@app.route('/api/analytics/revenue-data', methods=['GET'])
def get_revenue_data():
    """Get revenue data by event type"""
    try:
        query = """
            SELECT 
                e.event_type,
                SUM(e.entry_fee * COALESCE(reg_counts.reg_count, 0)) as total_revenue,
                COUNT(DISTINCT e.id) as event_count,
                AVG(e.entry_fee * COALESCE(reg_counts.reg_count, 0)) as avg_revenue
            FROM events e
            LEFT JOIN (
                SELECT event_id, COUNT(*) as reg_count
                FROM event_registrations
        ...
        revenue_data = execute_query(query)
        
        # Monthly revenue trend
        monthly_query = """
            SELECT 
                DATE_TRUNC('week', e.date_time) as week,
                SUM(e.entry_fee * COALESCE(reg_counts.reg_count, 0)) as weekly_revenue
            FROM events e
            LEFT JOIN (
                SELECT event_id, COUNT(*) as reg_count
                FROM event_registrations
            ...
            WHERE e.date_time >= CURRENT_DATE - INTERVAL '12 weeks'
            AND e.entry_fee > 0
            GROUP BY DATE_TRUNC('week', e.date_time)
            ORDER BY week ASC
        """
        
        monthly_data = execute_query(monthly_query)
        
        return jsonify({
            "success": True,
            "revenue_by_type": revenue_data or [],
            "weekly_revenue": monthly_data or []
        })
        
    except Exception as e:
        log_error(f"Error getting revenue data: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
...
def get_analytics_insights():
    ...
    try:
        ...
        top_events = execute_query(top_events_query)
        ...
        popular_games_query = """
            ...
            FROM events e
            LEFT JOIN event_registrations r ON e.id = r.event_id
            WHERE e.game_title IS NOT NULL AND e.game_title != ''
            GROUP BY e.game_title
            ORDER BY total_registrations DESC
            LIMIT 5
        """
        
        popular_games = execute_query(popular_games_query)
        ...
        peak_times = execute_query(peak_times_query)
        
        return jsonify({
            "success": True,
            "top_events": top_events or [],
            "popular_games": popular_games or [],
